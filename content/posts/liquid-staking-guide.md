...
> 💡 **参与流动性质押前需要先拥有 ETH。** 还没有 ETH？通过 [币安注册](https://www.bsmkweb.cc/register?ref=BT123)（邀请码 **BT123**）或 [欧易注册](https://www.promooboost.com/join/60895497) 购买后提现到钱包即可开始质押。
...
1. 你存入 1 ETH → 收到 1 stETH
2. 一个月后：1 stETH ≈ 1.003 ETH（累积了质押收益）
3. 一年后：1 stETH ≈ 1.035 ETH（累积了年化收益）
...
质押流程：ETH → Lido → stETH，拿到 stETH 后可以分别用于以下方向：
...
1. 用户存入 ETH，节点运营商存入 8 ETH（vs ETH）→ 两者共同组成 16 ETH 的迷你池
2. 迷你池 → 运行以太坊验证节点（32 ETH 全额）
3. 节点运营商获得额外奖励（作为运营补偿）
4. 用户获得质押基础收益
...
- **方案 A：纯质押** —— ETH → EtherFi → eETH（持有基础质押收益 ~3.3%）
- **方案 B：质押 + 原生再质押** —— ETH → EtherFi → eETH → 自动再质押到 EigenLayer（基础收益 + 再质押奖励 ~5-8%）
- **方案 C：质押 + 自定义再质押** —— ETH → EtherFi → 选择特定 AVS 再质押（基础收益 + 选定的 AVS 收益，风险水平可调）
...
操作流程：
1. 存入 ETH → Lido 获取 stETH
2. 将 stETH 存入 Aave V3 作为抵押品
3. 借出稳定币（USDC/DAI）——抵押率控制在 30-40%
4. 用借出的稳定币购买更多 ETH → 开始循环

> **注意：** 循环借贷会放大风险——如果 ETH 价格下跌，抵押率快速上升，可能面临清算。初学者不建议使用循环策略。

**保守方案（推荐）：**
将 stETH 存入 Aave → 不借出任何资产 → 赚取 Aave 的供应收益（APR 约 0.5-2% 取决于利用率）。**总收益 ≈ 3.5%（质押）+ 1%（Aave 供应）= 4.5%**。
...
1. ETH → Lido → stETH → 存入 EigenLayer 原生再质押
2. 再质押后可以依次获得：再质押积分、Eigen 代币空投、AVS 服务费分红，以及累计 AVS 积分带来的未来空投预期

**总预期收益 ≈ 3.5%（基础）+ 2-5%（再质押奖励 + 积分激励）**

**操作路径 B（通过 EtherFi 原生）：**

1. ETH → EtherFi → eETH → 自动再质押到 EigenLayer（协议原生支持）
2. 再质押后叠加：基础收益 + 再质押奖励 + 积分
...
1. 1 stETH → 1 wstETH（按当前汇率包装）
2. stETH 余额增加 → wstETH 对 ETH 的汇率上升
...
**核心决策框架：先问自己「你质押 ETH 的目的是什么？」，再选择对应方向：**

| 你的目的 | 选择协议 | 使用的代币 | 操作方式 | 特点 | 预期 APR |
|:--------|:-------|:----------|:--------|:-----|:--------:|
| 被动持有 | Lido | stETH | 质押后持有即可 | 最省心 | ~3.5% |
| DeFi 组合 | Lido | stETH | stETH → 借贷/LP | 中等操作 | ~4-12% |
| 再质押收益 | EtherFi | eETH | eETH → EigenLayer | 最高收益 | ~5-8% |
...
> 💡 **还没有 ETH？** 通过 [币安注册](https://www.bsmkweb.cc/register?ref=BT123)（邀请码 **BT123**）购买 ETH 后提现到钱包，即可开始你的流动性质押之旅。也可通过 [欧易注册](https://www.promooboost.com/join/60895497) 获取 ETH。
